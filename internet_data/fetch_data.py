...
from internet_data.tmp_data import *
...
class fetch_data:
    ...
    def get_orders(self ,start ,end):
        link = self.server_dns + "dinnersys_beta/backend/backend.php?cmd=select_other&cafet=true&&esti_start=" + \
            start + "&esti_end=" + end
        f = requests.get(link, cookies=self.cookie)
        resp = remove_bom(f.text)
        ret = json.loads(resp)#self.tmp.get_orders())
        return ret
    ...
    def get_dish(self):
        ...
        f = requests.get(link, cookies=self.cookie)
        resp = remove_bom(f.text)
        ret = json.loads(resp)#self.tmp.get_dish())
        return ret
